...
import com.example.fas.repositories.services.serviceImpl.exceptions.role.*;
import com.example.fas.repositories.services.RoleService;
...
public class RoleServiceImp implements RoleService {
    ...
    @Override
    @Transactional
    public RoleResponseDto updateRole(long id, RoleUpdateDto roleUpdate) {
        validateIdRole(roleUpdate.getRoleIdUpdate());
        validateIdRole(id);
        if (id != roleUpdate.getRoleIdUpdate()) {
            throw new IdInvalidException("Role ID in path and body do not match");
        }
        Role role = roleRepository.findById(roleUpdate.getRoleIdUpdate())
                .orElseThrow(() -> new RoleIdNotFoundException("Role ID not found"));
        ...
        if (!roleUpdate.getDescriptionUpdate().equals(role.getDescription())
                && !roleUpdate.getDescriptionUpdate().isBlank()) {
    ...
    public RoleResponseDto getRoleById(Long id) {
        validateIdRole(id);
        return roleMapper
                .toDto(roleRepository.findById(id).orElseThrow(() -> new RoleIdNotFoundException("Role ID not found")));
    }
     ...
     * @throws RoleDescriptionInvalidException if the role description is null or
     *                                         empty
     *
     */
    @Override
    public void validateRole(RoleRequestDto roleRequestDto) {
        if (roleRequestDto == null)
            throw new RoleInvalidException("Invalid role update");
        if (roleRequestDto.getRoleName() == null || roleRequestDto.getRoleName().trim().isEmpty()
                || roleRequestDto.getRoleName().isEmpty())
        ...
        if (roleRequestDto.getDescription() == null || roleRequestDto.getDescription().isEmpty()
                || roleRequestDto.getDescription().trim().isEmpty())
     ...
     * @throws IdInvalidException              if the role ID is null or less than
     *                                         or equal to zero
     * @throws RoleNameInvalidException        if the role name is null or empty
     * @throws RoleDescriptionInvalidException if the role description is null or
     *                                         empty
    ...
    public RoleResponseDto getRoleByName(String roleName) {
        Role roleOptional = roleRepository.findByRoleName(roleName.toUpperCase());
        if (roleOptional != null) {
            return roleMapper.toDto(roleOptional);
        } else {
